...
# def evaluate_feat_grid(pde_layer, latent_grid, t_seq, z_seq, x_seq, mins, maxs, pseudo_batch_size): # crey's delete
def evaluate_feat_grid(channel_list, pde_layer, latent_grid, t_seq, z_seq, x_seq, mins, maxs, pseudo_batch_size): # crey's create
    ...
    # phys_channels = ["p", "b", "u", "w"] # crey's delete
    phys_channels = channel_list # crey's program
    channel_num = len(channel_list) # crey's program
    ...
    for idx in tqdm(range(n_iters)):
        ...
        for name, chan_id in zip(phys_channels, range(channel_num)):
...
# def calculate_flow_stats(pred, hres, visc=0.0001): # crey's delete
def calculate_flow_stats(args, pred, hres, visc=0.0001): # crey's program
    data = pred
    # uw = np.transpose(data[2:4,:,:,1:1+args.eval_zres], (1, 0, 2, 3)) # crey's delete
    uw = np.transpose(data[2:4,:,:,:args.eval_zres], (1, 0, 2, 3)) # crey's create
...
# def export_video(args, res_dict, hres, lres, dataset): # crey's delete

def export_video(args, channel_list, res_dict, hres, lres, dataset, interp_data): # crey's create
    """Export inference result as a video.
    """
    # phys_channels = ["p", "b", "u", "w"] # crey's delete
    phys_channels = channel_list # crey's create
    if dataset:
        ...
        # calculate_flow_stats(args, pred, hres)       # Warning: only works with pytorch > v1.3 and CUDA >= v10.1 # crey's delete
    ...
    for idx, name in enumerate(phys_channels):
        ...
        interp_frames = interp_data[idx]

        # loop over each timestep in pred_frames
        max_val = np.max(hres_frames)
        min_val = np.min(hres_frames)

        # crey's start: ssim and psnr

        ssim_model_total = ssim(hres_frames, pred_frames)
        psnr_model_total = psnr(hres_frames, pred_frames, data_range=(max_val-min_val))

        ssim_interp_total = ssim(hres_frames, interp_frames)
        psnr_interp_total = psnr(hres_frames, interp_frames, data_range=(max_val-min_val))

        with open(os.path.join(args.save_path, "ssim_psnr.txt"), "a") as file:
            file.write("SSIM_model_{}    : {}\n".format(name,ssim_model_total))
            file.write("PSNR_model_{}    : {}\n".format(name,psnr_model_total))
            file.write("SSIM_interp_{}    : {}\n".format(name,ssim_interp_total))
            file.write("PSNR_interp_{}    : {}\n".format(name,psnr_interp_total))
...
def model_inference(args, lres, pde_layer):
    ...
    unet = UNet3d(in_features=channel_num, out_features=args.lat_dims, igres=igres,
    ...
    #                               args.eval_pseudo_batch_size) # crey's delete

    res_dict = evaluate_feat_grid(channel_list, pde_layer, latent_grid, t_seq, z_seq, x_seq, mins, maxs,
...
# crey's start: linear interp data fuction

def get_linear_hres(args, channel_list):
    
    dataset = loader.RB2DataLoader(channel_list=channel_list, 
    ...
    nt_hres, nz_hres, nx_hres = args.eval_tres, args.eval_zres, args.eval_xres

    nt_lres = nt_hres // args.downsamp_t
    nz_lres = nz_hres // args.downsamp_xz
    nx_lres = nx_hres // args.downsamp_xz

    interp = RegularGridInterpolator(
            (np.arange(nt_lres), np.arange(nz_lres), np.arange(nx_lres)),
            values=lres.transpose(1, 2, 3, 0), method='linear')

    lres_coord = np.stack(np.meshgrid(np.linspace(0, nt_lres-1, nt_hres),
                                          np.linspace(0, nz_lres-1, nz_hres),
                                          np.linspace(0, nx_lres-1, nx_hres),
                                          indexing='ij'), axis=-1)
    interp_data = interp(lres_coord).transpose(3, 0, 1, 2)  
...
def main():
    ...
    channels = args.channels # crey's program
    channel_list = channels.split(", ") # crey's program
    ...
    #     nx=args.eval_xres, nz=args.eval_zres, nt=args.eval_tres, n_samp_pts_per_crop=1,
    #     lres_interp=args.lres_interp, lres_filter=args.lres_filter, downsamp_xz=args.eval_downsamp_xz, downsamp_t=args.eval_downsamp_t,
    #     normalize_output=args.normalize_channels, return_hres=True) # crey's delete

    dataset = loader.RB2DataLoader(channel_list=channel_list, 
        data_dir=args.data_folder, data_filename=args.eval_dataset,
        nx=args.eval_xres, nz=args.eval_zres, nt=args.eval_tres, n_samp_pts_per_crop=1,
        lres_interp=args.lres_interp, lres_filter=args.lres_filter, downsamp_xz=args.eval_downsamp_xz, downsamp_t=args.eval_downsamp_t,
        normalize_output=args.normalize_channels, return_hres=True) # crey's create

    # extract data
    hres, lres, _, _ = dataset[0]

    # crey's start: linear interp data

    interp_data = get_linear_hres(args, channel_list)
    ...
    if args.normalize_channels:
        ...
        mean = std = None
    # pde_layer = get_rb2_pde_layer(mean=mean, std=std, prandtl=args.prandtl, rayleigh=args.rayleigh) # crey's delete

    pde_layer = crey_get_rb2_pde_layer(channels=args.channels, eqn_names=args.eqn_names, eqn_strs=args.eqn_strs, variable_values=args.variable_values,
    ...
    # save video
    # export_video(args, res_dict, hres, lres, dataset) # crey's delete
    export_video(args, channel_list, res_dict, hres, lres, dataset, interp_data) # crey's create
