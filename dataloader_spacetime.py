...
class RB2DataLoader(Dataset):
    ...
    #              return_hres=False, lres_filter='none', lres_interp='linear'): # crey's delete

    def __init__(self, channel_list, data_dir="./", data_filename="./data/rb2d_ra1e6_s42.npz",
                 nx=128, nz=128, nt=16, n_samp_pts_per_crop=1024,
                 downsamp_xz=4, downsamp_t=4, normalize_output=False, normalize_hres=False,
                 return_hres=False, lres_filter='none', lres_interp='linear', edge_sampling_mode=False): # crey's program
        ...
        # self.data = np.stack([npdata['p'], npdata['b'], npdata['u'], npdata['w']], axis=0) # crey's delete

        # crey's program

        npdata_list = []
        for channel in channel_list:
            npdata_list.append(npdata[channel])
        self.data = np.stack(npdata_list, axis=0)
    ...
    def __getitem__(self, idx):
        ...
        if self.edge_sampling_mode == False:
            ...
            point_num = 0
            flag = True
            new_point_coord, new_point_value = [], []

            while flag: 
                point_coord = np.random.rand(self.n_samp_pts_per_crop, 3) * (self.scale_hres - 1)
                point_value = interp(point_coord)

                for i in range(self.n_samp_pts_per_crop):
                    if point_value[i][1] != 0.0:
                        new_point_coord.append(point_coord[i])
                        new_point_value.append(point_value[i])
                        point_num += 1
                    if point_num == 512:
                        flag = False
                        break

            point_coord = np.array(new_point_coord).astype(np.float32)
            point_value = np.array(new_point_value).astype(np.float32)
