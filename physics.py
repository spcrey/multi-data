...
def get_rb2_pde_layer(mean=None, std=None, t_crop=2., z_crop=1., x_crop=2., prandtl=1., rayleigh=1e6, use_continuity=False):
    ...
    in_vars = 't, x, z'
    out_vars = 'p, b, u, w' # crey's delete
...
def crey_get_rb2_pde_layer(channels, eqn_names, eqn_strs, variable_values,  mean=None, std=None, use_continuity=False):  # crey's create
    ...
    """
    # constants
    # P = (rayleigh * prandtl)**(-1/2) # crey's delete
    # R = (rayleigh / prandtl)**(-1/2) # crey's delete
    # set up variables and equations
    in_vars = 't, x, z'
    # out_vars = 'p, b, u, w' # crey's delete
    ...
    # ]
    # # a name/identifier for the equations
    # eqn_names = ['transport_eqn_b', 'transport_eqn_u', 'transport_eqn_w'] # crey's delete
    # end crey variable_values["nx"]
    ...
    # if use_continuity:
    #     # eqn_strs.append(f'{nx} * dif(u, x) + {nz} * dif(w, z)') # crey's delete
    #     eqn_strs.append(f'{variable_values["nx"]} * dif(u, x) + {variable_values["nz"]} * dif(w, z)')
    #     eqn_names.append('continuity')
    ...
    if (mean is not None) or (std is not None):
        ...
        if not (hasattr(mean, '__len__') and hasattr(mean, '__len__')):
            raise TypeError(("mean and std must be arrays of len 4. instead they are {} and {}"
                             .format(type(mean), type(std))))
        # if not (len(mean) == 4 and len(std) == 4): # crey's delete
        if not (len(mean) == channel_num and len(std) == channel_num): # crey's program
